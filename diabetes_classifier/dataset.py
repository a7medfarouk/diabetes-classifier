...
def load_datasets() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    df_brfss2015  = pd.read_csv(RAW_DATA_DIR / "diabetes_binary_health_indicators_BRFSS2015.csv")
    df_brfss2021  = pd.read_csv(RAW_DATA_DIR / "diabetes_binary_health_indicators_BRFSS2021.csv")
...
def split_data(
    ...
    train, val  = train_test_split(train, test_size=validation_size, random_state=random_state, stratify=train[target_col])
...
def main(output_path: Path = INTERIM_DATA_DIR):
    ...
    train_brfss,      val_brfss,      test_brfss      = split_data(df_brfss_merged, target_col="Diabetes_binary")
    train_prediction, val_prediction, test_prediction = split_data(df_prediction,   target_col="diabetes")
