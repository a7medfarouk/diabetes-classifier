...
def load_validation_sets() -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    df_validation_prediction = pd.read_csv(
        INTERIM_DATA_DIR / "validation_prediction_dataset.csv"
    )
...
def encode_brfss(
    ...
    val = pd.get_dummies(
        val, columns=["MentHlth_binned", "PhysHlth_binned"], drop_first=True, dtype=int
    )
    test = pd.get_dummies(
        test, columns=["MentHlth_binned", "PhysHlth_binned"], drop_first=True, dtype=int
    )
...
def encode_prediction(
    ...
    train = pd.get_dummies(
        train, columns=["gender", "smoking_history"], drop_first=True, dtype=int
    )

    # align val and test to train columns
    val = pd.get_dummies(
        val, columns=["gender", "smoking_history"], drop_first=True, dtype=int
    )
    test = pd.get_dummies(
        test, columns=["gender", "smoking_history"], drop_first=True, dtype=int
    )
    val, _ = val.align(train, join="right", axis=1, fill_value=0)
    test, _ = test.align(train, join="right", axis=1, fill_value=0)
...
# ──────────────────────────────
def balance(
    X_train: pd.DataFrame, y_train: pd.Series, categorical_features: list
...
def main(output_path: Path = PROCESSED_DATA_DIR):
    ...
    # 2. Scale
    train_prediction, val_prediction, test_prediction, _ = scale_prediction(
        train_prediction, val_prediction, test_prediction
    )
    train_brfss, val_brfss, test_brfss, _ = scale_brfss(
        train_brfss, val_brfss, test_brfss
    )

    # 3. Discretize
    train_brfss, val_brfss, test_brfss = discretize_brfss(
        train_brfss, val_brfss, test_brfss
    )

    # 4. Encode
    train_brfss, val_brfss, test_brfss = encode_brfss(
        train_brfss, val_brfss, test_brfss
    )
    train_prediction, val_prediction, test_prediction = encode_prediction(
        train_prediction, val_prediction, test_prediction
    )

    # 5. Feature Interactions
    train_brfss, val_brfss, test_brfss = feature_interactions_brfss(
        train_brfss, val_brfss, test_brfss
    )
    train_prediction, val_prediction, test_prediction = feature_interactions_prediction(
        train_prediction, val_prediction, test_prediction
    )
    ...
    pred_cat_indices = [
        i
        for i, col in enumerate(X_train_prediction.columns)
        if col not in pred_num_cols
    ]

    brfss_cat_indices = [
        i for i, col in enumerate(X_train_brfss.columns) if col != "BMI"
    ]

    # 7. Balance train only
    X_train_brfss, y_train_brfss = balance(
        X_train_brfss, y_train_brfss, brfss_cat_indices
    )
    X_train_prediction, y_train_prediction = balance(
        X_train_prediction, y_train_prediction, pred_cat_indices
    )
    ...
    pd.concat([X_train_brfss, y_train_brfss], axis=1).to_csv(
        output_path / "featured_train_brfss.csv", index=False
    )
    ...
    pd.concat([X_train_prediction, y_train_prediction], axis=1).to_csv(
        output_path / "featured_train_prediction.csv", index=False
    )
